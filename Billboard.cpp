...
bool InitBillboard(void)
{
	...
	for (int i = 0; i < MAX_BILLBOARD; i++)
	{
		...
		billboard[i].RotationZswitch = false;
		billboard[i].angle = 0.f;
		//
	}
	return true;
}
...
void UpdateBillboard(void)
{
	...
	for (int i = 0; i < MAX_BILLBOARD; i++)
	{
		if (billboard[i].ISBillboard)
		{
			...
			if (billboard[i].RotationZswitch)
			{
				D3DXMatrixRotationZ(&billboard[i].MtxBillboardRotationZ, billboard[i].angle*D3DX_PI / 180.f);
				D3DXMatrixMultiply(&billboard[i].MtxBillboard, &billboard[i].MtxBillboardRotationZ, &billboard[i].MtxBillboard);
			}
				D3DXMATRIX mtxView;
				mtxView = GetView();
				mtxView._14 = 0.0f;
...
void DestroyBillboard(int Billboardnum)
{
		...
		billboard[Billboardnum].BillboardXYZ.x = 500.f;
		...
		billboard[Billboardnum].RotationZswitch = false;
		billboard[Billboardnum].angle = 0.f;
		//
}
...
void SetBillboardAngleZ(int Billboardnum,float angle)
{
	billboard[Billboardnum].angle = angle;
	billboard[Billboardnum].RotationZswitch = true;
}
